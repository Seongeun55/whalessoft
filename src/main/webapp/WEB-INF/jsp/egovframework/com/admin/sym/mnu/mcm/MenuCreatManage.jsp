...
function selectMenuCreat(vAuthorCode) {
	document.menuCreatManageForm.authorCode.value = vAuthorCode;
   	document.menuCreatManageForm.action = "<c:url value='/sym/mnu/mcm/AdminMenuCreatSelect.do'/>";
   	document.menuCreatManageForm.submit();
}
		...
		<tbody>
			<c:forEach var="result" items="${list_menumanage}" varStatus="status">
			  <tr>
			    ...
			    <td>
			       <a class="btn02" href="<c:url value='/sym/mnu/mcm/AdminMenuCreatSelect.do'/>?authorCode='<c:out value="${result.authorCode}"/>'"  onclick="selectMenuCreat('<c:out value="${result.authorCode}"/>'); return false;">메뉴생성</a>
			    </td>
			  </tr>
			 </c:forEach>
		</tbody>
