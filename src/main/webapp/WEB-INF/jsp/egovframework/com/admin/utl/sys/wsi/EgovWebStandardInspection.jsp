...
function fn_egov_submit_WebStandardInspection(form, nNmm){
	...
	*/
	...
	if(form.rdoUri[0].checked == true){
		form.action='/PageLink.do?link=utl/sys/wsi/EgovWebStandardInspectionUriDirect';
	}else{
		form.action='/PageLink.do?link=utl/sys/wsi/EgovWebStandardInspectionUri';
	}
 ...
 ******************************************************** */
 function fn_egov_submit_WebStandardInspectionLink(form){
	...
	if(form.uri.value == ''){
	 ...
	 if(form.rdoUri[0].checked == true){
		 document.formHidden.action = "/PageLink.do?link=utl/sys/wsi/EgovWebStandardInspectionUriDirectLink";
	 }else{
...
<%
	for(int i=1 ; i<10; i++){ 
%>
<form name="webInspection" method="post" action="/PageLink.do?link=utl/sys/wsi/EgovWebStandardInspectionUri" target="ifr_hidden">
