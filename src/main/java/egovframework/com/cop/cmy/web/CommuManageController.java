...
public class CommuManageController {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(CommuManageController.class);

	@Resource(name = "CommuManageService")
	private CommuManageService CommuManageService;

	@Resource(name = "CommuBBSMasterService")
	private CommuBBSMasterService CommuBBSMasterService;

	@Resource(name = "CommuMasterService")
	private CommuMasterService CommuMasterService;
	...
	public String selectCmmntyMain(@ModelAttribute("searchVO") CommunityVO cmmntyVO, ModelMap model, HttpServletRequest request) throws Exception {
...
//		String tmplatCours = cmmntyService.selectCmmntyTemplat(cmmntyVO);
		...
		Map<String, Object> map = CommuManageService.selectCommuInf(cmmntyVO);
		...
		List<BoardMasterVO> bbsResult = CommuBBSMasterService.selectCommuBBSMasterListMain(bbsVo);
	...
	public String insertCmmntyUserBySelf(@ModelAttribute("cmmntyUser") CommunityUser cmmntyUser, ModelMap model) throws Exception {
		...
		if (isAuthenticated) {
			...
			// 승인요청 처리
			// ---------------------------------------------
			retVal = CommuManageService.checkCommuUserDetail(cmmntyUser);

			// 요청건이 없을 경우
			if (!retVal.equals("EXIST")) {

				CommuManageService.insertCommuUserRqst(cmmntyUser);
				retVal = egovMessageSource.getMessage("comCopCmy.commuMain.joinMember.info.success"); // 가입신청이
																										// 정상처리되었습니다.
			} else {
	...
	public String deleteCmmntyUserBySelf(@ModelAttribute("cmmntyUser") CommunityUserVO cmmntyUserVO, ModelMap model) throws Exception {
		...
		Boolean isCommuAdmin = CommuManageService.selectIsCommuAdmin(userVO);

		// 관리자는 탈퇴할 수 없음.
		String resultMsg = "";
		if (isAuthenticated && !isCommuAdmin) {
			cmmntyUserVO.setEmplyrId(user == null ? "" : EgovStringUtil.isNullToString(user.getUniqId()));
			CommuManageService.deleteCommuUser(cmmntyUserVO);
			resultMsg = egovMessageSource.getMessage("comCopCmy.commuMain.deleteMember.info.success"); // 탈퇴신청이
																										// 정상처리되었습니다.
		} else {
	...
	public String selectCommuUserList(@ModelAttribute("searchVO") CommunityUserVO cmmntyUserVO, ModelMap model) throws Exception {
		...
		Map<String, Object> map = CommuManageService.selectCommuUserList(cmmntyUserVO);
	...
	public String insertCommuUser(@ModelAttribute("searchVO") CommunityUserVO cmmntyUserVO, ModelMap model) throws Exception {
		...
		if (isAuthenticated && isCommuAdmin) {
			...
			CommuManageService.insertCommuUser(cmmntyUserVO);
		}
	...
	public String insertCommuUserAdmin(@ModelAttribute("searchVO") CommunityUserVO cmmntyUserVO, ModelMap model) throws Exception {
		...
		if (isAuthenticated && isCommuAdmin) {
			cmmntyUserVO.setLastUpdusrId(user == null ? "" : EgovStringUtil.isNullToString(user.getUniqId()));
			CommuManageService.insertCommuUserAdmin(cmmntyUserVO);
		}
	...
	public String deleteCommuUserAdmin(@ModelAttribute("searchVO") CommunityUserVO cmmntyUserVO, ModelMap model) throws Exception {
		...
		cmmntyVO = CommuMasterService.selectCommuMaster(cmmntyVO);
		...
		if (isAuthenticated && isCommuAdmin) {
			cmmntyUserVO.setLastUpdusrId(user == null ? "" : EgovStringUtil.isNullToString(user.getUniqId()));
			CommuManageService.deleteCommuUserAdmin(cmmntyUserVO);
		}
